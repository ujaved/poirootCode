...
PREFIX_RANGE_SENT = [240,242,244,246,248]
RIOT_ASN = 47065
SENTINEL = 0
TEST = 1
SENT_ROUND = 10
...
def populateDB_sent(c,num,poison_round,status):

	pfx = '184.164.' + str(num) + '.0/24'

	print "---------------------------------"
	print "populateDB_sent"
	...
	mux = MUX_NAMES[PREFIX_RANGE_SENT.index(num)]
...
def populateDB_ftr_sent(c,num,poison_round):

	pfx = '184.164.' + str(num) + '.0/24'

	print "---------------------------------"
	print "populateDB_ftr_sent"
	print "time: " + str(int(time.time()))
	print "prefix: " + str(pfx)
	...
	for feed in feeds:	
		...
		for rv in locs:
			...
			c.execute('insert into TRGraph values (?,?,?,?,?,?,?,?,?)', \
				   (pfx, unix_time, poison_round,last_poison_time, feed, path_str, hops_str,str(rv),-1))
...
def main():
	...
	for mux in MUX_NAMES:
		...
		pfx = num2pfx[PREFIX_RANGE_SENT[pfx_idx]]
		pfx.poisonList([RIOT_ASN])
		announce_sentinel(pfx,mux)
	...
	while True:
		if num_ftr_query>=NUM_FTR:
			...
			for num in PREFIX_RANGE_SENT:				
				if poisonRound%SENT_ROUND==0 and sent_used is True:
					populateDB_sent(c,num,poisonRound,SENTINEL)
				else:
					populateDB_sent(c,num,poisonRound,TEST)
				conn.commit()
			...
			for num in PREFIX_RANGE_SENT:
				if poisonRound%SENT_ROUND==0 and sent_used is True:
					populateDB_ftr_sent(c,num,poisonRound)
				else:
					populateDB_ftr_sent(c,num,poisonRound)
				conn.commit()
