...
def getNextPoisoning(num):
	...
	while (not pfx2VPBeingPoisoned[num].cur_path or nextAS2BePoisoned in opts.blacklist):
		...
		pfx2VPBeingPoisoned[num].curPoisonings.clear()
		
		dealWithPathExhaustion(num)
		nextAS2BePoisoned = pfx2VPBeingPoisoned[num].getNextAS2BePoisoned()

	return pfx2VPBeingPoisoned[num].getPoisonSet()
		...
		for rv in locs:
			...
			if (not update.path) is False:
				...
				if tup is None:
					...
					if poison_round==0 or status==SENTINEL:
					...
					if poison_round > 0 and status==TEST:
						if str(rv)==vpPoisoned:
...
def create_db():
	...
	c.execute("create table if not exists ASGraph (prefix text, unix_time int, poison_round int, status int, \
                   mux_seen text, poisonedASes text, feed text, observedPath text, ipPath text, vpoint text, thisVPoisoned int)")
...
PREFIX_RANGE = range(240,249,2)
RIOT_ASN = 47065
SENTINEL = 0
TEST = 1
#MUX_NAMES = ['UW', 'WISC', 'GATECH', 'PRINCE', 'CLEMSON']
MUX_NAMES = ['WISC', 'GATECH', 'PRINCE', 'CLEMSON']
muxASToName = dict()
...
def main():
	...
	global conn
	#c = create_db()
	conn = sqlite3.connect('../data/poison_data/bfs.db2012-07-20 15:03:40.407590')
	c = conn.cursor()
	...
	while True:
		populateDB(c,PREFIX_RANGE[-1],poisonRound,SENTINEL,"")
		conn.commit()
