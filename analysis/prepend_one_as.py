	...
	for feed in feeds:
		...
		for rv in locs:
			...
			c.execute('insert into ASGraph values (?,?,?,?,?,?,?,?,?)', \
						  (pfx, unix_time, dtime, \
						   numPrepend, mux, feed, str(update.path), str(update.hops),rv))
...
def main():
	...
	#announce 250-254 from one mux and 240 from all muxesscreen emacs help
