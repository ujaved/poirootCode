...
def dealWithPathExhaustion(num):
	...
	while ((pfx2VPBeingPoisoned[num].poisonIdx==0) or (pfx2VPBeingPoisoned[num].rv in pfx2VPAlreadyPoisoned[num]) \
		       or (pfx2PoisVPNotSeenCount[num]==3)):
	...
	if pfx2PoisVPNotSeenCount[num]==3:
...
def getNextPoisoning(num,poison_round,sentinel_just_used):
	...
	while ((not pfx2VPBeingPoisoned[num].cur_path) or (nextAS2BePoisoned in opts.blacklist) \
		       or (pfx2VPBeingPoisoned[num].numPoisonNoEffect==2)):
	...
	for feed in feeds:	
		...
		for rv in locs:
			...
			if (not update.path) is False:
				...
				if tup is None or len(update.path) < 3 or update.path[0] != tup[0][0]:
					update.path = []
				else:
					filpath = tup[0]
					mux = tup[1]
...
TEST = 1
SENT_ROUND = 10
MUX_NAMES = ['WISC', 'GATECH', 'PRINCE', 'UW','CLEMSON']
muxASToName = dict()
